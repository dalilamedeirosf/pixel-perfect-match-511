  ...
  head: () => ({
    meta: [
      { title: "Sessão Diagnóstico — Felipe Amaral | Craque em Construção" },
      {
        name: "description",
        content:
          "Relatório de Performance — Diagnóstico completo do atleta Felipe Amaral.",
      },
    ],
  }),
});
...
function Page({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-white text-black mx-auto my-8 shadow-md w-full max-w-[820px] min-h-[1100px] px-16 py-20 font-serif">
      {children}
    </div>
  );
}
...
function Index() {
  return (
    <main className="min-h-screen bg-neutral-200 py-8 px-4">
      {/* Page 1 — Capa */}
      <Page>
        <div className="flex flex-col items-center justify-center text-center min-h-[900px]">
          <p className="text-sm tracking-[0.3em] mb-12">SESSÃO DIAGNÓSTICO</p>
          <img src={logo} alt="CC" className="h-48 w-auto mb-10" />
          <h1 className="text-2xl font-bold tracking-[0.3em]">CRAQUE EM CONSTRUÇÃO</h1>
          <p className="mt-6 text-sm tracking-[0.3em]">MENTORIA ESPORTIVA</p>
        </div>
      </Page>

      {/* Page 2 — Capa do relatório */}
      <Page>
        <img src={cover} alt="Craque em Construção" className="w-full mb-10" />
        <h2 className="text-xl font-bold mb-2">RELATÓRIO DE PERFORMANCE</h2>
        <p className="mb-6">DIAGNÓSTICO COMPLETO DO ATLETA</p>
        <p className="text-2xl font-bold mb-6">Felipe amaral</p>
        <p className="italic mb-10">Relatório exclusivo para a família de Felipe amaral</p>
...
function Section({ s }: { s: { num: string; title: string; body: string } }) {
  return (
    <section className="mb-10">
      <h3 className="font-bold mb-4">
        {s.num} {s.title}
      </h3>
      <p className="text-justify leading-relaxed text-[15px]">{s.body}</p>
    </section>
  );
}
